...
$wdp_products = [
    ['type' => 'WDP FAST', 'price' => 27900, 'desc' => 'Proses cepat'],
    ['type' => 'WDP SLOW', 'price' => 24000, 'desc' => 'Proses 3-4 hari']
];

$diamond_packages = [
    ['diamonds' => '3', 'price' => 1300], ['diamonds' => '5', 'price' => 1900], ['diamonds' => '10', 'price' => 4000],
    ['diamonds' => '20', 'price' => 6100], ['diamonds' => '28', 'price' => 8800], ['diamonds' => '36', 'price' => 10800],
    ['diamonds' => '44', 'price' => 13500], ['diamonds' => '50', 'price' => 15200], ['diamonds' => '59', 'price' => 17100],
    ['diamonds' => '74', 'price' => 21000], ['diamonds' => '85', 'price' => 24100], ['diamonds' => '128', 'price' => 35300],
    ['diamonds' => '148', 'price' => 42300], ['diamonds' => '153', 'price' => 44900], ['diamonds' => '172', 'price' => 48500],
    ['diamonds' => '222', 'price' => 59500], ['diamonds' => '257', 'price' => 70200], ['diamonds' => '296', 'price' => 79400],
    ['diamonds' => '344', 'price' => 94100], ['diamonds' => '370', 'price' => 105000], ['diamonds' => '408', 'price' => 112400],
    ['diamonds' => '500', 'price' => 142800], ['diamonds' => '568', 'price' => 154500], ['diamonds' => '600', 'price' => 159100],
    ['diamonds' => '716', 'price' => 191300], ['diamonds' => '875', 'price' => 230100]
];
?>
<!DOCTYPE html>
...
<head>
    ...
    <title>Mobile Legends - <?php echo htmlspecialchars($site_name); ?></title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .product-amount { color: #7289da; font-size: clamp(1.2rem, 4vw, 1.5rem); font-weight: 800; }
        .product-price { color: #ffcc00; font-size: clamp(1rem, 3.5vw, 1.2rem); font-weight: 700; margin-top: 10px; }
        .product-desc { font-size: clamp(0.65rem, 2.5vw, 0.7rem); color: rgba(255, 255, 255, 0.6); margin-top: 8px; }
...
<body>
    <nav class="navbar"><div class="container-fluid px-3 px-md-4"><div class="d-flex justify-content-between align-items-center w-100"><div class="d-flex align-items-center gap-3"><button class="menu-toggle" type="button" data-bs-toggle="offcanvas" data-bs-target="#sidebarOffcanvas"><i class="bi bi-list"></i></button><div><h3 class="mb-0" style="color: #7289da; font-size: clamp(1.2rem, 5vw, 1.5rem);"><i class="bi bi-controller me-2"></i><?php echo htmlspecialchars($site_name); ?></h3><small class="text-muted d-none d-sm-block">Mobile Legends Store</small></div></div><div class="user-info"><span class="user-name"><i class="bi bi-person-circle"></i><span class="d-none d-sm-inline"><?php echo htmlspecialchars($username); ?></span><span class="d-inline d-sm-none"><?php echo substr(htmlspecialchars($username), 0, 10); ?></span></span><a href="../../logout.php" class="btn-dashboard"><i class="bi bi-box-arrow-right"></i><span class="d-none d-sm-inline">Logout</span></a></div></div></div></nav>
    ...
    <div class="col-lg-10"><div class="main-content"><a href="store.php" class="back-button"><i class="bi bi-arrow-left"></i> Kembali ke Store</a>
        <div class="store-container"><div class="text-center mb-4"><i class="bi bi-joystick" style="font-size: clamp(3rem, 8vw, 4rem); color: #7289da;"></i><h1 class="mt-2" style="color: #7289da; font-size: clamp(1.5rem, 5vw, 2rem);">MOBILE LEGENDS TOPUP</h1><p class="text-muted">Diamond & WDP Topup Service</p></div>
        
        <div class="section-header"><span><i class="bi bi-lightning-charge"></i> WDP MLBB</span><span class="section-note">Fast & Slow</span></div>
        <div class="products-grid" id="wdpGrid"><?php foreach ($wdp_products as $index => $product): ?><div class="product-card" data-type="wdp" data-index="<?php echo $index; ?>" onclick="selectProduct('wdp', <?php echo $index; ?>)"><div class="product-amount"><?php echo $product['type']; ?></div><div class="product-price">Rp <?php echo number_format($product['price'], 0, ',', '.'); ?></div><div class="product-desc"><?php echo $product['desc']; ?></div></div><?php endforeach; ?></div>
        
        <div class="section-header"><span><i class="bi bi-gem"></i> DIAMOND PACKAGES</span><span class="section-note">Instant</span></div>
        <div class="products-grid" id="diamondGrid"><?php foreach ($diamond_packages as $index => $package): ?><div class="product-card" data-type="diamond" data-index="<?php echo $index; ?>" onclick="selectProduct('diamond', <?php echo $index; ?>)"><div class="product-amount"><?php echo $package['diamonds']; ?> 💎</div><div class="product-price">Rp <?php echo number_format($package['price'], 0, ',', '.'); ?></div></div><?php endforeach; ?></div>
        
        <div class="form-section"><h3 class="form-title"><i class="bi bi-person-badge"></i> INFORMASI PEMESANAN</h3><div class="row"><div class="col-md-6"><div class="form-group"><label class="form-label">ID MOBILE LEGENDS</label><input type="text" class="form-control" id="mlbbId" placeholder="Contoh: 123456789"><div class="info-box"><i class="bi bi-info-circle"></i> Masukkan ID Mobile Legends Anda</div></div></div><div class="col-md-6"><div class="form-group"><label class="form-label">NICKNAME / IGN</label><input type="text" class="form-control" id="nickname" placeholder="Contoh: ProPlayer123"><div class="info-box"><i class="bi bi-info-circle"></i> Masukkan nickname / in-game name</div></div></div><div class="col-md-6"><div class="form-group"><label class="form-label">NOMOR WHATSAPP</label><input type="text" class="form-control" id="whatsappNumber" placeholder="Contoh: 628123456789"><div class="info-box"><i class="bi bi-info-circle"></i> Pastikan nomor WhatsApp aktif</div></div></div><div class="col-md-6"><div class="form-group"><label class="form-label">ZONE ID (Optional)</label><input type="text" class="form-control" id="zoneId" placeholder="Contoh: 1234"><div class="info-box"><i class="bi bi-info-circle"></i> Zone ID membantu proses lebih cepat</div></div></div></div></div>
        
        <div class="order-summary"><h3 class="summary-title">Ringkasan Pesanan</h3><div class="summary-item"><span>Jenis Produk:</span><span class="summary-value" id="summaryProductType">-</span></div><div class="summary-item"><span>Detail Produk:</span><span class="summary-value" id="summaryProductDetail">-</span></div><div class="summary-item"><span>ID MLBB:</span><span class="summary-value" id="summaryMlbbId">-</span></div><div class="summary-item"><span>Nickname:</span><span class="summary-value" id="summaryNickname">-</span></div><div class="summary-item"><span>WhatsApp:</span><span class="summary-value" id="summaryWhatsapp">-</span></div><div class="summary-item total"><span>Total Pembayaran:</span><span class="summary-value" id="summaryTotal">Rp 0</span></div></div>
    ...
    <script>
        const wdpProducts = <?php echo json_encode($wdp_products); ?>;
        const diamondPackages = <?php echo json_encode($diamond_packages); ?>;
        ...
        function selectProduct(type, index) {
            ...
            selectedType = type; selectedProduct = type === 'wdp' ? wdpProducts[index] : diamondPackages[index];
        ...
        function updateSummary() {
            ...
            if (selectedProduct && selectedType) {
                document.getElementById('summaryProductType').textContent = selectedType === 'wdp' ? 'WDP MLBB' : 'Diamond MLBB';
                document.getElementById('summaryProductDetail').textContent = selectedType === 'wdp' ? selectedProduct.type : selectedProduct.diamonds + ' Diamonds';
            ...
            document.getElementById('summaryMlbbId').textContent = mlbbId || '-';
        ...
        function checkOrderReady() {
            ...
            const isValid = selectedProduct && mlbbId.length >= 6 && nickname.length >= 3 && /^(\+62|62|0)8[1-9][0-9]{6,9}$/.test(whatsapp.replace(/\s/g, ''));
        ...
        function processOrder() {
            ...
            let productInfo = selectedType === 'wdp' ? `WDP ${selectedProduct.type}` : `${selectedProduct.diamonds} Diamonds`;
            let totalPrice = selectedProduct.price;
            const message = `*ORDER MLBB*

Halo Admin, saya ingin memesan top up MLBB:
...
Mohon informasi pembayaran selanjutnya. Terima kasih!`;
        ...
        document.getElementById('mlbbId').addEventListener('input', () => { updateSummary(); checkOrderReady(); });
        document.getElementById('nickname').addEventListener('input', () => { updateSummary(); checkOrderReady(); });
